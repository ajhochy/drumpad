...
- **A5 partial (labels only — NOT resolved):** `skibidi_tomtomlet` → `trono_tomtomlet` ("Trono Tomtomlet", 👑🥁) renamed across iOS `Drumrots.json`, web `js/drumrots.js`, and `art/drumrots/` + asset catalog; the `Skibidi_Toilet.png` source reference was dropped from the web entry. **BUT the rendered card art (`art/drumrots/trono_tomtomlet.webp` and its iOS imageset) is still a drum-ified Skibidi Toilet — clearly recognizable.** Renaming the label is cosmetic; **A5 is not resolved until the portrait is regenerated** to an original "porcelain tom god" design with no Skibidi/toilet character. `brainrots/OG/Skibidi_Toilet.png` source also still on disk. Also added `ITSAppUsesNonExemptEncryption=false` (export-compliance) and BpmStepper a11y labels.

...
- **A2. RESOLVED (2026-05-26).** Re-synthesize in AVAudioEngine — port the `js/audio.js` oscillator/noise synthesis to `AVAudioPCMBuffer`s. No sample files, no audio licensing. Confirms planner decision D6.
- **A3. RESOLVED (2026-05-26).** Generate a fresh icon + full asset set in the established SP-808 voxel style via the same image pipeline as `scripts/generate-drumrots.py`. The icon must be an **original mascot / brand mark — no recognizable third-party character** (keeps the icon itself clear of the A4/A5 IP risk). Output: 1024×1024 master + `AppIcon.appiconset` + a derived favicon that also closes web issue [#6](https://github.com/ajhochy/drumpad/issues/6). Implemented under issue GitHub #46.
- **A4. RESOLVED w/ caveat (2026-05-26).** Portraits are AI-generated by `scripts/generate-drumrots.py` (`google/gemini-2.5-flash-image` via OpenRouter), treated as original/owned. **Caveat:** the generator *edits a `parodyImg` source image* and is instructed to keep the result "instantly recognizable as a parody of the original," so each portrait is a **derivative work** of its source. Before Phase 10: (a) confirm the image model's commercial-use terms, (b) confirm the `parodyImg` source images are themselves owned/licensable, (c) retain generation prompts/seeds. Overlaps A5.
- **A5. DIRECTION set (2026-05-26) — not legal advice.** User's position: treat drumrots as parody/derivative works (cf. Weird Al). Guardrails agreed: (1) the app **icon** and brand marks are fully original (A3); (2) a pre-submission content-review pass scrubs anything reading as a *specific named/trademarked* character (e.g. "Skibidi"), keeping the generic "Italian-brainrot" meme-genre flavor; (3) consult IP counsel on borderline names before submission — parody is a fact-specific *copyright* defense (weaker under *trademark*), and Apple App Review (Guideline 5.2) can reject third-party-IP use regardless of legal merit. Weird Al is illustrative but he *licenses* his parodies rather than relying on fair use.

A1–A3 are resolved; A4 is resolved with a provenance caveat and A5 has an agreed direction — both finalize during the Phase 10 content/IP review. None block any earlier phase.

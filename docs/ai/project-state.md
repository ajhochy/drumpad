...
## #58 MIDI diagnostic overlay cherry-picked from main (2026-05-27)
- Cherry-picked `main` commit `c66c69e` onto `release/ios16-only`. Adds `MIDIInputManager.RecentEvent` + `@Published recentEvents` ring (cap 8); logs every note-on, mapped or unmapped, so the diagnostic distinguishes "no CoreMIDI delivery" from "delivery works, GM-mapping gap". `MIDIDiagnosticOverlay` SwiftUI component lives at `Features/Play/MIDIDiagnosticOverlay.swift`. **DEBUG:** floats top-right of LCD highway via `PlayView`'s `.overlay(alignment: .topTrailing)` wrapped in `#if DEBUG`. **Release on ios16:** ported into ios16's redesigned Service Panel as a styled `DisclosureGroup("RECENT ACTIVITY")` inside `midiPane`, after the Bluetooth pair row, matching the existing rowShell/lcdPanel aesthetic. Two fixups during cherry-pick: (a) `MIDIDiagnosticOverlay.swift` renamed `SPColor.lcdFg` → `SPColor.lcdFG` to match ios16's SPColor casing; (b) hand-ported the `showRecentMIDI` state + DisclosureGroup into ios16's `midiPane` because main's flat `Form` and ios16's panel sidebar diverge structurally. **Verified on ios16 sim:** `xcodebuild build` SUCCEEDED + 45/45 tests green. DEBUG-overlay sim screenshot captured during main verification; the ios16 disclosure uses the same `MIDIDiagnosticOverlay` component so visual parity holds.

## #59 MIDIKit adoption cherry-picked from main (2026-05-27)
- Cherry-picked `main` commit `e4edced` onto `release/ios16-only`: `Drumrot/MIDI/MIDIInputManager.swift` rewritten over **orchetect/MIDIKit 1.1.0** via the umbrella `SwiftMIDI` product. Public API preserved (`@Published sources` / `@Published activity` / `var onNote` / `func start()`); PlayView and SettingsView call-sites untouched. Network MIDI kept on manual `MIDINetworkSession.default()` for build-1 parity. SPM dep + Package.resolved committed for CI determinism. Fixes the iOS-16 UMP silent-receive root cause behind #57 (MIDIKit transparently falls back to legacy `MIDIInputPortCreateWithBlock` on iOS 16/17). See `docs/ai/decisions.md` 2026-05-27 entry for the MIDIKit 0.x → 1.x rename trap (`SwiftMIDI` not `MIDIKitIO`; `NotificationHandler` is 1-arg). **Verified on ios16 sim:** `xcodebuild build` SUCCEEDED + 45/45 tests green (iPad Pro 11-inch M5 sim, CODE_SIGNING_ALLOWED=NO). Hardware verification on the older-iPad target (sideload to TD-50X iPad, strike kick → highway pulses + audio + scoring) is the real-device gate this branch was forked for and remains pending.

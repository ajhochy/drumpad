...
## Next step
Active parallel-branch run (2026-05-27): three pending issues to land on **both** `main` (iOS 17+ SwiftData) and `release/ios16-only` (iOS 16.6+ UserDefaults-persistence), in order:
1. **#59 MIDIKit adoption** — DONE locally on `feature/midikit-adoption`; commit + push + PR to main pending → wait for user merge → cherry-pick to `release/ios16-only` (file is conflict-free per the divergence map in `IOS16_BRANCH.md`).
2. **#58 on-screen MIDI diagnostic overlay** — adds `@Published recentEvents` ring buffer (cap 8) on `MIDIInputManager`; DEBUG floating panel top-right of `PlayView`; Release disclosure under `SettingsView` → MIDI & In → "Recent activity". Layers on top of #59.
3. **#60 external audio toggle** — `externalAudioMode: Bool = false`; on `main` adds field to `@Model AppSettings` (`Data/AppSettings.swift`), on `release/ios16-only` adds field to the Codable `AppSettings` struct inside `Data/PersistenceStore.swift`. Early-return gate atop `DrumAudioEngine.play(lane:velocity:)`; SettingsView Audio pane row. SettingsView is a known cherry-pick conflict zone (binding plumbing diverges: `@Query` on main vs `@EnvironmentObject PersistenceStore` on ios16).

Constraints still active: no direct `project.pbxproj` edits (hook blocks; Xcode crashes); SPM additions only via Xcode UI; no `release/ios16-only` → `main` merge; no `CFBundleVersion` bump / TestFlight upload without explicit user authorization.

Remaining human/hardware gates (unchanged from 2026-05-26): real iPad latency + USB/Network/BLE MIDI (including TD-50X UMP-fallback validation for #57/#59), VoiceOver, iPad app on Apple silicon Mac, App Store Connect record/screenshots/preview (#51), TestFlight upload + external tester smoke (#52), A4/A5 content/IP review before submission.
